...
if ! docker-compose -f docker-compose.yml -f docker-compose.formula.yml exec -T postgres /bin/bash -c "until echo > /dev/tcp/postgres/5432; do sleep 1; done; psql xpub xpub -c \"SELECT 'public.entities'::regclass\""; then
